...
import { LayoutDashboard, Film, Zap, CreditCard, User, X, Rocket } from "lucide-react"
...
export function CreatorSidebar({ isMobile = false, isOpen = false, onClose }: CreatorSidebarProps) {
  const menuItems = [
    ...
    { icon: Zap, label: "Funding", href: "/creator/funding" },
      ...
      <nav className="space-y-2 flex-1">
        {menuItems.map((item) => (
          <Link
            key={item.label}
            href={item.href}
            className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors text-sidebar-foreground"
